...
public open class MysqlService : Service {
     ...
     */
    public fun transaction(isolation: Isolation? = null, body: (tr: MysqlTransConn) -> Unit): Unit =
        this.conn.transaction(isolation = isolation) { body(MysqlTransConn(conn = it, ser = this.ser)) }
}
