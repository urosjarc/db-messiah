...
class OracleService(conf: HikariConfig, val ser: OracleSerializer) {
    ...
    fun transaction(isolationLevel: IsolationLevel? = null, body: (tr: OracleTransConn) -> Unit) =
        this.service.transaction(isoLevel = isolationLevel) { body(OracleTransConn(conn = it, ser = this.ser)) }
}
