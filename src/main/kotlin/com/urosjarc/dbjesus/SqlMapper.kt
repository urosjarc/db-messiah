...
class SqlMapper(val sqlTypeSerializers: List<SqlTypeSerializer<*>>) {
    fun getDbType(kclass: KClass<*>): String = this.sqlTypeSerializers.first { it.kclass == kclass }.dbType
    ...
    fun getObjProperties(obj: Any, primaryKey: String): ObjProperties {
        ...
        for (kp in obj::class.declaredMemberProperties) {
            ...
            val serializer = this.sqlTypeSerializers.firstOrNull { prop.kclass == it.kclass }
            if (serializer == null) throw SqlMappingException("Serializer missing for: KClass<${prop.kclass}")
            val objProp = ObjProperty(
                name = kp.name,
                value = prop.get(obj),
                property = prop,
                serializer = serializer as SqlTypeSerializer<Any>
            )
        ...
        if (primaryKeyProp == null) throw SqlMappingException("Primary key '$primaryKey' not found in: $obj")
        return ObjProperties(primaryKey = primaryKeyProp, list = objProps)
    }

    fun decode(resultSet: ResultSet, columnInt: Int, decodeInfo: DecodeInfo): Any? {
        val jdbcType = resultSet.metaData.getColumnType(columnInt)

        for (tser in this.sqlTypeSerializers) {
            if (tser.jdbcType.ordinal == jdbcType && decodeInfo.kparam.kclass == tser.kclass) {
                return tser.decoder(resultSet, columnInt, decodeInfo)
            }
        }

        throw SqlMappingException("Serializer missing for: $decodeInfo")
    }
    ...
    fun <T : Any> decode(kclass: KClass<T>, resultSet: ResultSet): T {
        ...
        try {
            ...
            throw SqlMappingException("KClass<$kclass> can't be constructed with arguments: $args", e)
        }
    }
}
