...
class Test_DerbySerializer : Test_Serializer() {
    ...
    override fun `test selectTable page`() {
        assertEquals(
            ...
            expected = Query(sql = "SELECT * FROM ${wrap("main")}.${wrap("Parent")} ORDER BY ${wrap("pk")} OFFSET 45 ROWS FETCH FIRST 15 ROWS ONLY")
        )
    }
    @Test
    ...
    override fun `test createTable`() {
        assertEquals(
            ...
            actual = Query(sql = """CREATE TABLE "main"."Parent" ("pk" INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, "col" VARCHAR(100) NOT NULL)"""),
        )
        assertEquals(
        ...
        assertEquals(
            ...
            actual = Query(sql = """CREATE TABLE "main"."Other" ("pk" INTEGER PRIMARY KEY, "cascades" INTEGER NOT NULL, "canBeNull" INTEGER, "notNull" INTEGER NOT NULL, "unique" INTEGER NOT NULL UNIQUE, FOREIGN KEY ("cascades") REFERENCES "main"."Other" ("pk") ON DELETE CASCADE)"""),
        )
    }
    ...
    override fun `test dropSchema`() {
        assertEquals(
            ...
            expected = Query(sql = "DROP SCHEMA \"main\"")
        )
        assertEquals(
