...
class Test_SqliteSerializer : Test_Serializer() {
    ...
    override fun `test createTable`() {
        assertEquals(
            ...
            actual = Query(sql = """CREATE TABLE IF NOT EXISTS "Parent" ("pk" INTEGER PRIMARY KEY AUTOINCREMENT, "col" VARCHAR(100) NOT NULL)"""),
        )
        assertEquals(
            expected = this.ser.createTable(Child::class),
            actual = Query(sql = """CREATE TABLE IF NOT EXISTS "Child" ("pk" INTEGER PRIMARY KEY, "fk" INTEGER NOT NULL, "col" VARCHAR(100) NOT NULL, FOREIGN KEY ("fk") REFERENCES "Parent" ("pk"))"""),
        )
        assertEquals(
            expected = this.ser.createTable(Other::class),
            actual = Query(sql = """CREATE TABLE IF NOT EXISTS "Other" ("pk" INTEGER PRIMARY KEY, "cascades" INTEGER NOT NULL, "canBeNull" INTEGER, "notNull" INTEGER NOT NULL, "unique" INTEGER NOT NULL UNIQUE, FOREIGN KEY ("cascades") REFERENCES "Other" ("pk") ON DELETE CASCADE)"""),
        )
    }
